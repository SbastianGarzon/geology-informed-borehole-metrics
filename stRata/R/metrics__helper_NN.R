#' Summarize and Reshape Neural Network Metrics by Hyperparameter
#'
#' This function processes a data frame of Neural Network performance metrics (`metrics_by_fold`) by summarizing the mean and standard deviation
...
#'
#' @param metrics_by_fold A data frame containing Neural Network performance metrics, with columns for `mtry`, `fold`, and various metrics (e.g., accuracy, error)
...
#'
#' @export
nn_metrics_by_hyper_parameter <- function(metrics_by_fold){

  summary_df <- metrics_by_fold %>% select(-c(file)) %>%
    group_by(lstm_units,num_heads,learning_rate)  %>%
  ...
  metrics_by_hyper_parameter <- summary_df %>%
    ...
    pivot_longer(
      cols = -c(lstm_units,num_heads,learning_rate),  # Exclude 'mtry' from the long format
