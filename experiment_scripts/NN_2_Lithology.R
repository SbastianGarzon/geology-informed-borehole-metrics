...
units_per_borehole_coord <- read.csv2(file.path("..","Data","units_per_borehole_coord.csv"),sep=";")
...
df_labeled_rec <- df_labeled  %>% select(-c(nr,mv,end,x_28992,y_28992,depth,zmk_litho,original_index,lith_plot))

recipe <- recipe(strat_inter ~ ., data = df_labeled_rec) %>%
  step_impute_mode(all_nominal(), -all_outcomes()) %>% # Impute mode for unknown levels in nominal predictors
  step_impute_median(all_numeric_predictors()) %>% 
  step_novel(all_nominal(), -all_outcomes()) %>% # Handle unknown levels in nominal predictors
  step_dummy(all_nominal(), -all_outcomes(),one_hot = TRUE) %>% # One-hot encode all nominal predictors
  step_dummy(all_outcomes(),one_hot = TRUE) %>%                  # One-hot encode the target variable
  step_normalize(c("zm","zmk","zmk_ml","lutum_pct","sh","org")) %>%    # Normalize numeric predictors
...
output_folder <- file.path(experiment_name,"NN2")
...
for(i in folds){
  ...
  df_train_NN1 <- df_train %>% select(-c(nr,mv,end,x_28992,y_28992,depth,zmk_litho,original_index,lith_plot))
  df_validation_NN1 <- df_validation  %>% select(-c(nr,mv,end,x_28992,y_28992,depth,zmk_litho,original_index,lith_plot))
  df_test_NN1 <- df_test  %>% select(-c(nr,mv,end,x_28992,y_28992,depth,zmk_litho,original_index,lith_plot))
  ...
  for(num_heads_att in num_heads){
    for(num_lstm_units in num_lstm){
      for(learning_rate in learning_rate_list){
        ...
        output_file <- paste0("fold_",i,"_LSTM_",num_lstm_units,"_Num_heads_",num_heads_att,"_LR_",learning_rate_str,".png")
        ...
        test_predictions <- model %>% tf$keras$Model$predict(test_data$value_ragged_tensor)
        col_names <- test_data$target_names
        
        predictions_test <- predictions_to_df(test_predictions,df_test,col_names = col_names)
...
write.csv2(combined_df, file.path(experiment_name,"NN2_tuning_results.csv"), row.names = FALSE)
