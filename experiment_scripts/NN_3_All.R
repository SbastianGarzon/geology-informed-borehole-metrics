...
df_labeled_rec <- df_labeled  %>% select(-c(nr,mv,end,zmk_litho,original_index,lith_plot,fold))
...
recipe <- recipe(strat_inter ~ ., data = df_labeled_rec) %>%
  ...
  step_normalize(c("zm","zmk","zmk_ml","lutum_pct","sh","org","x_28992","y_28992","depth")) %>%    # Normalize numeric predictors
...
output_folder <- file.path(experiment_name,"NN3")
...
for(i in folds){
  ...
  df_train_NN1 <- df_train %>% select(-c(nr,mv,end,zmk_litho,fold,original_index,lith_plot))
  df_validation_NN1 <- df_validation  %>% select(-c(nr,mv,end,fold,zmk_litho,original_index,lith_plot))
  df_test_NN1 <- df_test  %>% select(-c(nr,mv,end,zmk_litho,fold,original_index,lith_plot))
...
write.csv2(combined_df, file.path(experiment_name,"NN3_tuning_results.csv"), row.names = FALSE)
